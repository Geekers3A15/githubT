 ...
 */
public class ModifierWeblogController implements Initializable {
    ...
    @FXML
    private TableColumn<contenu, String> actionCol;
    @FXML
    ...
    public void initData(user u, int id_weblog) {
        this.currentUser = u;
        System.out.println("identifiant : " + u.getId_user().get());
        System.out.println("identifiant weblog : " + id_weblog);
        ...
        weblog w = ws.getWeblogById(id_weblog);
        contenu c = cs.getContenuById(id_weblog);

        ObservableList<contenu> contenu = FXCollections.observableArrayList();

        contenu = cs.getAllContenuByWeblog(id_weblog);
        ...
        Callback<TableColumn<contenu, String>, TableCell<contenu, String>> cellFactory
                = (param) -> {
                    final TableCell<contenu, String> cell = new TableCell<contenu, String>() {
                ...
                public void updateItem(String item, boolean empty) {
                    ...
                    if (empty) {
                        ...
                        supprimerbutton.setOnAction(event -> {
                            contenu c = getTableView().getItems().get(getIndex());

                            cs.deleteContenuByIdContenu(c.getId_contenu().get());
                            initData(currentUser, id_weblog);
        ...
        tfTitre.setText(w.getTitre().get());
        boxCatégorie.setValue(w.getCategorie().get());
    ...
    @FXML
    private void handle_modifierTitre(MouseEvent event) {
        weblogService ws = weblogService.getInstance();
        weblog w = ws.getWeblogById(id_weblog);
        if (event.getSource() == modifierTitre) {
            w.setTitre(tfTitre.getText());
            ws.updateTitreWeblog(w);
            Image img = new Image("assets/tick.png", 50, 50, false, false);
            Notifications notificationsBuilder = Notifications.create()
                    .title("Tire du weblog")
                    .text("Titre modifier avec succée")
                    .graphic(new ImageView(img))
                    .hideAfter(Duration.seconds(6))
                    .position(Pos.TOP_LEFT);
            notificationsBuilder.darkStyle();
