...
public class weblogController implements Initializable {
            ...
            for (weblog w : weblogs) {
                for (user ur : users) {
                    if (ur.getId_user().get() == w.getId_artiste().get()) {
                        ...
                        fXMLLoader.setLocation(getClass().getResource("/views/cardWeblog.fxml"));
                        VBox vbox = fXMLLoader.load();
                        CardWeblogController cardweblogwontroller = fXMLLoader.getController();
                        cardweblogwontroller.setData(w, ur);
                        if (column == 3) {
                            column = 0;
                            ++row;
                        }
    ...
    @FXML
    private void handle_ajoutWeblog(MouseEvent event) {
        weblogService ws = weblogService.getInstance();
        weblog w = new weblog();
        try {
            ...
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(getClass().getResource("/views/ajouterWeblog.fxml"));
            Parent page2 = loader.load();
            AjouterWeblogController controller = loader.getController();
            controller.initData(currentUser);
            Scene scene = new Scene(page2);
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setScene(scene);
    ...
    private void handle_modifierWeblog(MouseEvent event) {
        try {
            ...
            controller.initData(currentUser, identification);
            Scene scene = new Scene(page2);
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setScene(scene);
            ...
            Logger.getLogger(ModifierWeblogController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void handle_Chercher(MouseEvent event) {
        ...
        if (texte.equals("")) {
            ...
            weblogService ws = weblogService.getInstance();
            List<weblog> weblogs = ws.getWeblogByTitreObservable(texte, currentUser.getId_user().get());
            userService us = userService.getInstance();
            List<user> users = us.getAllUsers();
            if (weblogs.isEmpty()) {
                    ...
                    for (weblog w : weblogs) {
                        for (user u : users) {
                            if (u.getId_user().get() == w.getId_artiste().get()) {
                                ...
                                cardweblogcontroller.setData(w, u);
                                if (column == 3) {
                                    column = 0;
                                    ++row;
                                }
    ...
    private void handle_afficherBtn(MouseEvent event) {
        weblogService ws = weblogService.getInstance();
        List<weblog> weblogs = ws.getAllWeblogsOfUser(currentUser.getId_user().get());
        userService us = userService.getInstance();
        List<user> users = us.getAllUsers();
        int column = 0;
        int row = 1;
